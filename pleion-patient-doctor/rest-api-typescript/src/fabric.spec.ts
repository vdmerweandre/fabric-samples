...
describe('Fabric', () => {
  ...
  describe('getContracts', () => {
    it('gets the symptom and qscc contracts from the network', async () => {
  ...
  describe('evatuateTransaction', () => {
    ...
    it('throws an SymptomExistsError an symptom already exists error occurs', async () => {
      mockTransaction.evaluate.mockRejectedValue(
        new Error('The symptom JSCHAINCODE already exists')
      );

      await expect(async () => {
        await evatuateTransaction(mockContract, 'txn', 'arga', 'argb');
      }).rejects.toThrow(SymptomExistsError);
    });

    it('throws an SymptomNotFoundError if an symptom does not exist error occurs', async () => {
      mockTransaction.evaluate.mockRejectedValue(
        new Error('The symptom JSCHAINCODE does not exist')
      );

      await expect(async () => {
        await evatuateTransaction(mockContract, 'txn', 'arga', 'argb');
      }).rejects.toThrow(SymptomNotFoundError);
    });
