...
/**
 * Get the network which the symptom transfer sample chaincode is running on
 *
...
/**
 * Get the symptom transfer sample contract and the qscc system contract
 *
 * The system contract is used for the liveness REST endpoint
 */
export const getContracts = async (
  network: Network
): Promise<{ symptomContract: Contract; qsccContract: Contract }> => {
  const symptomContract = network.getContract(config.chaincodeName);
  const qsccContract = network.getContract('qscc');
  return { symptomContract, qsccContract };
};
